...
const Residence = () => {
    ...
    async function handleDeleteRes() {
        //clean jotai residence object
        await setResidenceInfo(null);

        try {
            const res = await removeResidence();
        }
        catch (err) {
            console.log(err);
        }
        ...
        <>
            <Container className="flex">
                {residenceInfo.houseType === "" || residenceInfo.houseType === null || residenceInfo === null ?
                    ...
                    <Container>
                        <Row>
                            <p style={{ fontWeight: 'bold', fontSize: '2rem' }}>  Residence Information</p>
                        </Row>
                        <Row>
